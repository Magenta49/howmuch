document.addEventListener('DOMContentLoaded', function() {
    // 계산기 폼 요소 가져오기
    const calculatorForm = document.getElementById('funeral-calc');
    ...
    // 장례식 부의금 기준 데이터 (단위: 만원)
    const funeralGiftData = {
        // 가족/친척
        'family': {
            'seoul': { min: 5, max: 30, recommend: 20 },
            'gyeonggi': { min: 5, max: 30, recommend: 15 },
            'metro': { min: 5, max: 20, recommend: 10 },
            'province': { min: 5, max: 20, recommend: 10 }
        },
        // 망인의 친한 친구/학교 동기
        'close-friend': {
    ...
    calculatorForm.addEventListener('submit', function(e) {
        ...
        // 기본 금액 계산
        const baseAmount = funeralGiftData[relationship][region];
        ...
        explanation += `${getVenueText(venue)} 장례식장 유형, ${attendance === 'yes' ? '참석' : '불참'}을 고려한 금액입니다.`;
        
        if (previous > 0) {
            explanation += ` 이전에 받은 ${previous/10000}만원의 부의금도 고려하였습니다.`;
        }
    ...
    function saveCalculation(relationship, region, venue, attendance, previous, amount) {
        ...
        let calculations = JSON.parse(localStorage.getItem('funeralCalculations')) || [];
        ...
        // 로컬 스토리지에 저장
        localStorage.setItem('funeralCalculations', JSON.stringify(calculations));
    }
});